...
public class PedometerActivityReportView extends View {
	...
	private List<Pedometer_activity_detail> dataList;
	...
	public PedometerActivityReportView(Context context) {
		super(context);
		init(null);
	}

	public PedometerActivityReportView(Context context, AttributeSet attrs) {
		super(context, attrs);
		init(null);
	}

	public PedometerActivityReportView(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
		init(null);
	}

	public void init(List<Pedometer_activity_detail> dataList) {
	...
	public void drawStraightLine(Canvas c) {
		...
		// 数据背景分割线
		base = (getHeight() - interval_bottom) / 4;
		...
		paint_dottedline2.setColor(fineLineColor);
		Path path2 = new Path();
		path2.moveTo(0, getHeight() - interval_bottom - 2 * base);
		path2.lineTo(getWidth(), getHeight() - interval_bottom - 2 * base);
	...
	public void drawBrokenLine(Canvas c) {
		if (dataList != null) {
			...
			if (dataList.size() == 1) {
				...
				float y = getHeight() - interval_bottom - StringUtils.string2Int(dataList.get(0).getActive_hour()) * base;
				drawCircle(c, x, y, paint_circle);

			} else {
				for (int i = 0; i < dataList.size() - 1; i++) {
					Pedometer_activity_detail data1 = dataList.get(i);
					Pedometer_activity_detail data2 = dataList.get(i + 1);

					float x1 = base_h * i + interval_left_right / 2.0f;
					float y1 = getHeight() - interval_bottom - StringUtils.string2Int(data1.getActive_hour()) * base;

					float x2 = base_h * (i + 1) + interval_left_right / 2.0f;
					float y2 = getHeight() - interval_bottom - StringUtils.string2Int(data2.getActive_hour()) * base;
...
//					}
	...
	private void drawDate(Canvas c) {
		if (dataList != null) {
			...
			for (int i = 0; i < dataList.size(); i++) {
				Pedometer_activity_detail data = dataList.get(i);
				float x = base_h * i + interval_left_right / 2.0f;
				float y = getHeight() - interval_bottom / 2;
				// drawText(c, x, y, data);
				String dateStr = "";
				try {
					dateStr = dataList.get(i).getStep_date().substring(5);
				} catch (Exception e) {
				}
	 ...
	 * 刷新，重绘图
	 */
	public void refresh(List<Pedometer_activity_detail> dataList) {
		this.dataList = dataList;
		init(this.dataList);
		invalidate();
	}
