...
public class RestingHeartRateReportView extends View {
	...
	private List<Resting_heart_rate_detail> dataList;
	...
	public RestingHeartRateReportView(Context context) {
		super(context);
		init(null);
	}

	public RestingHeartRateReportView(Context context, AttributeSet attrs) {
		super(context, attrs);
		init(null);
	}

	public RestingHeartRateReportView(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
		init(null);
	}

	public void init(List<Resting_heart_rate_detail> dataList) {
	...
	public void drawBrokenLine(Canvas c) {
		if (dataList != null) {
			...
			if (dataList.size() == 1) {
				...
				float y = getHeight() - interval_bottom - StringUtils.string2Int(dataList.get(0).getHeartRate()) * base;
				drawCircle(c, x, y, paint_circle);

			} else {
				for (int i = 0; i < dataList.size() - 1; i++) {
					Resting_heart_rate_detail data1 = dataList.get(i);
					Resting_heart_rate_detail data2 = dataList.get(i + 1);
...
//					}
	...
	private void drawDate(Canvas c) {
		if (dataList != null) {
			...
			for (int i = 0; i < dataList.size(); i++) {
				Resting_heart_rate_detail data = dataList.get(i);
				float x = base_h * i + interval_left_right / 2.0f;
	 ...
	 * 刷新，重绘图
	 */
	public void refresh(List<Resting_heart_rate_detail> dataList) {
		this.dataList = dataList;
		init(this.dataList);
		invalidate();
	}
