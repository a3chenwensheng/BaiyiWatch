...
public class MediaPlayTools {
	...
	private void play(boolean isEarpiece , int seek) {
		...
		try {
			...
			AudioManager mAudioManager = (AudioManager) MyApplication.getInstance().getSystemService(MyApplication.getInstance().AUDIO_SERVICE);
			int streamVolume = 0;
			if (isEarpiece) {
				streamVolume = mAudioManager.getStreamVolume(AudioManager.STREAM_VOICE_CALL);
			}else {
	...
	public boolean stop () {
		...
		if(mListener != null) {
			mListener.onVoicePlayInterrupt();
		}
